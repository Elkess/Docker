...
Tick items off as you go. Don't skip ahead in a section — each box assumes
the ones above it are actually done, not just skimmed.
...
**Done when every box above is checked and you can walk through the whole
stack out loud — what each container does, why it's built that way, and
what would break if you removed any one constraint — without notes.**
